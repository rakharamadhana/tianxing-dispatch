...
/** Approved amount is what's actually paid out; falls back to reported until approved. */
function payoutAmount(row) {
  return row.status === 'approved' ? Number(row.approved_amount) || 0 : Number(row.reported_amount) || 0
}

function buildDetailSheet(workbook, name, rows, styles) {
  const sheet = workbook.addWorksheet(name)
  const header = ['日期時間', '司機', '回報金額', '核准金額', '狀態']
  sheet.columns = [
    { key: 'datetime', width: 18 },
    { key: 'driver', width: 16 },
    { key: 'reported', width: 14 },
    { key: 'approved', width: 14 },
    { key: 'status', width: 12 }
  ]
  ...
  sorted.forEach((row, index) => {
    ...
    excelRow.getCell(3).value = Number(row.reported_amount) || 0
    excelRow.getCell(4).value = Number(row.approved_amount) || 0
  ...
  totalRow.getCell(4).value = sorted.reduce((sum, row) => sum + payoutAmount(row), 0)
...
function buildYearMatrixSheet(workbook, name, yearRows, styles) {
  ...
  const monthly = MONTH_NAMES.map((label, index) => {
    ...
    return { label, count: rows.length, amount: rows.reduce((s, r) => s + payoutAmount(r), 0) }
  })
...
/** Summary (總表): the given month's fuel requests — driver totals + detail. */
export async function buildMonthlyFuelWorkbook({ records, branch, year, month }) {
  ...
  buildDriverTotalsSheet(workbook, `${branch} ${year}年${month}月油資總表`, monthRows, styles)
  buildDetailSheet(workbook, '明細', monthRows, styles)
  return workbook
}

/** Full Summary (全總表): the whole year — a monthly matrix + full detail. */
export async function buildYearlyFuelWorkbook({ records, branch, year }) {
  const yearRows = records.filter((row) => dateKey(row).startsWith(String(year)))

  const { workbook, styles } = await loadStyledWorkbook()
  buildYearMatrixSheet(workbook, `${branch} ${year}年度油資總表`, yearRows, styles)
  buildDetailSheet(workbook, '明細', yearRows, styles)
  return workbook
}

export async function downloadMonthlyFuelReport({ records, branch, year, month, filename }) {
  const workbook = await buildMonthlyFuelWorkbook({ records, branch, year, month })
  await downloadWorkbook(workbook, filename)
}

export async function downloadYearlyFuelReport({ records, branch, year, filename }) {
  const workbook = await buildYearlyFuelWorkbook({ records, branch, year })
  await downloadWorkbook(workbook, filename)
}
